...
import {
  createUpdateRoleSchemaFactory,
  type UpdateRoleFormType,
  DEFAULT_UPDATE_ROLE_FORM_VALUES,
...
export const UpdateRoleModal = ({
  ...
  const { t } = useLocale();
  const schema = createUpdateRoleSchemaFactory(t);
  ...
  const {
    ...
    defaultValues: DEFAULT_UPDATE_ROLE_FORM_VALUES,
    mode: "onBlur",
  ...
  }, [roleData, isOpen, reset]);

  const { mutate: updateRole, isPending } = useUpdateRole({
    onSuccess: () => {
      showToast(t("pages.roles.messages.updateSuccess"), "success");
      reset();
      onClose();
      onSuccess?.();
    }
  ...
  const onSubmit = (data: UpdateRoleFormType) => {
    if (!roleId) return;
    updateRole({ id: roleId, params: data });
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={t("pages.roles.modals.update.title")}
      size="md"
    >
      {isLoadingRole ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 my-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-800/50">
            ...
            <Button type="submit" variant="primary" loading={isPending}>
              {isPending
                ? t("pages.roles.actions.updating")
                : t("pages.roles.actions.update")}
            </Button>
          </div>
        </form>
      )}
